...
//		byte[] keyByte = "o#$%$^^%&DF3GH%&%^&b".getBytes();
		byte[] keyByte = StringUtil.getDefaultByte("o#$%$^^%&DF3GH%&%^&b");
	    Blowfish blowfish = new Blowfish();
	    blowfish.init(true, keyByte);
	    byte[] tt = blowfish.encrypt(s);
	    s = Base64Util.encode(tt);
	    return s;
	}
	public static String decode(String s){
		byte tempByte[] = Base64Util.decode(s);
//		byte[] keyByte = "o#$%$^^%&DF3GH%&%^&b".getBytes();
		byte[] keyByte = StringUtil.getDefaultByte("o#$%$^^%&DF3GH%&%^&b");
		Blowfish blowfish = new Blowfish();
		blowfish.init(true, keyByte);
		s = blowfish.decryptString(tempByte);
		return s;
	}
}
